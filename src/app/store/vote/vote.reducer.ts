...
export const voteReducer = createReducer(
  ...
  on(VoteActions.deleteVote, (state) => ({
    ...state,
    loading: true,
    error: null
  })),
  on(VoteActions.deleteVoteSuccess, (state) => ({
    ...state,
    loading: false,
    error: null
  })),
  on(VoteActions.deleteVoteFailure, (state, { error }) => ({
    ...state,
    loading: false,
    error
  })),
  on(VoteActions.alterVote, (state) => ({
    ...state,
    loading: true,
    error: null
  })),
  on(VoteActions.alterVoteSuccess, (state) => ({
    ...state,
    loading: false,
    error: null
  })),
  on(VoteActions.alterVoteFailure, (state, { error }) => ({
    ...state,
    loading: false,
    error
  }))
